...
function stripMargin(text: string) {
  ...
  let maxIndent = lines[0].length;
  for (const line of lines) {
    if (line.trim()) {
      maxIndent = Math.min(maxIndent, SPACE_REGEX.exec(lines[0])[0].length);
    }
  }
  return (
    lines
      .map((line) => {
        return line.substring(maxIndent);
      })
      .join("\n") + "\n"
  );
}
